...
MONGOMS_SKIP_MD5_CHECK environment variable`, () => {
    expect(new _MongoBinaryDownload.default({}).skipMD5).toBe(false);
    process.env.MONGOMS_SKIP_MD5_CHECK = '1';
    expect(new _MongoBinaryDownload.default({}).skipMD5).toBe(true);
  });
...
the same as in the reference result`, () => {
  ...
  the same as in the reference result`, () => {
    ...
    expect(du.checkMD5('', '')).rejects.toMatchInlineSnapshot(`[Error: MongoBinaryDownload: md5 check is failed]`);
  });
